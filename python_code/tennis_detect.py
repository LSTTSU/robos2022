...
class TennisDetect(object):
    ...
    def tennis_detect(self, img, video_return):
        ...
        if circles is not None:
            # extract the x, y, r of all detected circles
            ...
            for i in range(num):  # traverse all detected circles
                ...
                if height != 0 and width != 0:
                    tennis_color_mask = cv2.inRange(
                        detect_area, self.lower_range, self.higher_range)
                    num_point = np.sum(tennis_color_mask / 255)
                    rate[i] = num_point / (height * width)
                    img_out = cv2.circle(
                        img_out, (x[i], y[i] + self.cut_edge), r[i], (0, 255, 0), thickness=2)

            # select the circle with the maximum rate as the detected tennis
            i = np.argmax(rate)
        ...
        if video_return:  # if it needs to return the frame with the detected tennis
            img_out = cv2.circle(
                img_out, (x_pos, y_pos + self.cut_edge), radius, (0, 0, 255), thickness=10)
